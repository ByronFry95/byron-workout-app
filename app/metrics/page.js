...
export default function MetricsPage() {
  ...
  useEffect(() => {
    ...
    getMetricsHistory(user.uid).then(data => {
      setHistory(data)
      const latest = data[0]
      if (latest) setMeasurements(previous => ({ ...previous, ...latest, date: today() }))
